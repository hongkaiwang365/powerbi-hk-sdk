...
class Dataset
{
     ...
     * @param string|null $groupId   An optional group ID
     * @param bool|null   $notify    set if user recibe notify mail - $notify = ['notifyOption' => 'MailOnFailure','retryCount'=>3]
     *
     * @return Response
     */
    public function refreshDataset($datasetId, $groupId = null, $notify = [])
    {
        $url = $this->getRefreshUrl($groupId, $datasetId);
//        dump($notify);
//        dd($url);
        $response = $this->client->request(Client::METHOD_POST, $url, $notify);
        return $this->client->generateResponse($response);
    }

    /**
     * Notes:
     * @author:hongkai - 4/19/2023 3:57 PM
     * @param $url
     * @return Response
     */
